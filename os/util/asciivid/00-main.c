...
int main(int argc, char **argv) {
     ...
     * for files named <4-digit-hex>.TXT. For each, strtoi the basename into a number;
     * this is the index in the array (the frame number). At that location in the array,
     * write the starting cluster number of that file. We keep the whole dirent of one
     * of the files, so for each file we need to read, we just replace the starting cluster
     * in the dirent and pass to fat16_readfile().
     */
...
static void usage() {
    print("Usage: asciivid <directory>\n");
}
