...
# Check for a third argument
LDI_D .argv_buf+8      # D points at fourth argument pointer
...
LDI_A .argv_buf+8      # Get pointer to first arg into C
LDA_A_CH                        # |
LDI_A .argv_buf+9      # |
LDA_A_CL                        # |
...
CALL :ata_read_lba      # status returned in byte on heap
CALL :heap_pop_AL
...
.helpstr "Usage: ata-read {0/m|1/s} <lba 27:16> <lba 15:0> <dest-addr>\n\0"
...
.complete_str "Sector 0x%x%x%x%x loaded at 0x%x%x\n\0"
.error_str "Unable to read device, error flags: 0b%2\n\0"
