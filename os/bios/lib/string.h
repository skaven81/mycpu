...
// Case-insensitive string compare. Returns negative/zero/positive like strcmp.
// Both strings are compared as if uppercased; suitable for FAT16 8.3 filename
// lookup (on-disk names are always uppercase per the FAT16 spec).
extern int8_t strcasecmp(char *s1, char *s2);
